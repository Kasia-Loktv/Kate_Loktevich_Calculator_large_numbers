...
namespace Calculator_large_numbers
{
    class Program
    {
            ...
            while (true)
            {
                ...
                switch (op)
                {
                    case "+":
                        ...
                        {
                            int sum = ((int)(str1[i] - '0') + (int)(str2[i + lend] - '0') + rest);
                            str += (char)(sum % 10 + '0');
                            rest = sum / 10;
                        }
                    ...
                    case "-":
                        ...
                        n1_diff = str1.Length;
                        n2_diff = str2.Length;
                        int lend_diff = n1_diff - n2_diff;
                        int rest_diff = 0;

                        for (int i = n2_diff - 1; i >= 0; i--)
                        {
                            int sub = (((int)str1[i + lend_diff] - (int)'0') - ((int)str2[i] - (int)'0') - rest_diff);
                            if (sub < 0)
                            {
                                sub = sub + 10;
                                rest_diff = 1;
                        ...
                        {
                            if (str1[i] == '0' && rest_diff > 0)
                            {
                                str_diff += "9";
                                continue;
                            }
                            int sub = (((int)str1[i] - (int)'0') - rest_diff);
                            if (i > 0 || sub > 0)
                            {
                                str_diff += sub.ToString();
                            }
                    ...
                    case "*":
                        ...
                        for (i_mult = len1 - 1; i_mult >= 0; i_mult--)
                        {
                            ...
                            for (int j = len2 - 1; j >= 0; j--)
                            {
                                ...
                                int sum = n1_mult * n2_mult + result[i_n1 + i_n2] + rest_mult;
                                rest_mult = sum / 10;
                    ...
                    case "/":
                        ...
                        {
                            answer += (char)(temp / div + '0');

                            temp = (temp % div) * 10 + (int)(number[idx] - '0');
                            idx++;
                        }
                        answer += (char)(temp / div + '0');
